...
export function validateQuizConfig({ numQuestions, distribution }) {
  ...
  const mcq = Number.isFinite(dist.mcq) ? Number(dist.mcq) : undefined;
  const saq = Number.isFinite(dist.saq) ? Number(dist.saq) : undefined;
  const laq = Number.isFinite(dist.laq) ? Number(dist.laq) : undefined;
