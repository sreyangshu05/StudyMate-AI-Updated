...
// Unwrap the canonical backend envelope { success, data } once, here, so every
// consumer reads response.data.<payloadField> directly. The backend wraps all
// success responses with ok(res, payload) -> { success:true, data: payload };
// error responses keep their { success:false, error } shape (handled in catch).
// The `?? response.data` fallback lets test mocks that resolve { data: {...} }
// (without the envelope) keep working unchanged.
