...
const QuizGenerator = ({ selectedDocIds, onQuizGenerated }) => {
  ...
  const normalizeDistribution = (targetTotal, currentDistribution) => {
    const types = ['mcq', 'saq', 'laq'];
    const next = {
      mcq: Math.max(0, Number(currentDistribution.mcq) || 0),
      saq: Math.max(0, Number(currentDistribution.saq) || 0),
      laq: Math.max(0, Number(currentDistribution.laq) || 0),
    };
    let total = types.reduce((sum, type) => sum + next[type], 0);
    ...
    if (total < targetTotal) {
      next.mcq += targetTotal - total;
      return next;
    }

    let over = total - targetTotal;
    const priorities = ['laq', 'saq', 'mcq'];
    for (const type of priorities) {
      if (over <= 0) break;
      const reduction = Math.min(next[type], over);
      next[type] -= reduction;
      over -= reduction;
    }
    return next;
  ...
  const generateQuiz = async () => {
    ...
    const normalizedDistribution = normalizeDistribution(numQuestions, distribution);

    setGenerating(true);
    try {
      const response = await quizAPI.generate(selectedDocIds, numQuestions, normalizedDistribution);
      toast.success('Quiz generated successfully!');
      onQuizGenerated(response.data);
    } catch (error) {
      const message = error.response?.data?.error?.message || 'Failed to generate quiz';
      toast.error(message);
